 ...
 *
 * Con veinte puestos se encuentran con la vista; con ciento cuarenta, no. Y
 * quien busca no siempre sabe por dónde: unos se acuerdan del código, otros de
 * media palabra del nombre, otros de algo que está en la descripción. Se busca
 * en todo lo que la fila enseña en vez de obligar a elegir un campo.
 */
const busqueda = ref('')
...
const rows = computed(() => {
  ...
  return ordenados.value.filter((p) =>
    enPlano(`${p.code} ${p.name} ${p.description ?? ''} ${entityName(p.legalEntityId)}`).includes(
      q,
    ),
  )
})
...
<template>
  <div class="space-y-4">
    ...
    <UTable
      ...
      :empty="busqueda.trim() ? 'Ningún puesto coincide con esa búsqueda.' : 'No hay puestos.'"
    >
